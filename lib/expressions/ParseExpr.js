...
function fParseNumericLiteral(oLexer, oStaticContext) {
    ...
    if (aMatch) {
        ...
        if (aMatch[2] || aMatch[3])
            cType	= cXSDecimal;
		oLexer.next();
        // A double literal is binary by definition, so +sValue is what it
        // means. An integer or decimal literal is exact, and +sValue would
        // round it here — before evaluation has begun.
        return new cNumericLiteral(cType === cXSDouble
                ? new cType(+sValue)
                : new cType(cXSDecimal.parse(sValue)));
    }
};
