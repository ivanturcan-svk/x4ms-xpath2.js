...
/*
	Two untyped operands — XForms-compatible, declared in EXTENSIONS.md.

	XPath 2.0, section "General Comparisons" casts both operands to xs:string
	when both are untyped, and an instance without a schema makes every value
	untyped. A form comparing two number fields therefore compared text: "9"
	sorts after "10", so nine was not less than ten.

	Both operands are compared as numbers instead, but only when both are
	written the way a number is written:

		^-?(0|[1-9]\d*)(\.\d+)?$

	Ivan, 2026-08-12: "číslo sa štandardne nepíše do dátového poľa s vodiacimi
	nulami, a teda je to formátovaný string." A leading zero, a leading plus,
	a missing integer part or an exponent all mean the field holds formatting
	and the comparison stays textual. A trailing zero does not disqualify —
	otherwise 19.99 < 20.00, a money comparison, would fall back to text.
	...
	prescribes for that type. The comparison itself is exact: the canonical
	form is a decimal lexical form, so the value is what the author wrote and
	not the nearest double to it.

	The engine already made this call for the mixed case — one untyped operand
	is cast to the other side's type rather than to xs:string. This carries the
	same decision to the case where neither side has a type.
*/
var rComparisonExpr_canonicalNumber	= /^-?(0|[1-9]\d*)(\.\d+)?$/;

function fComparisonExpr_castUntypedPair(vLeft, vRight) {
	var oLeft	= cXSString.cast(vLeft),
		oRight	= cXSString.cast(vRight);
	if (rComparisonExpr_canonicalNumber.test(oLeft.toString().trim())
			&& rComparisonExpr_canonicalNumber.test(oRight.toString().trim()))
		return [cXSDecimal.cast(oLeft), cXSDecimal.cast(oRight)];
	return [oLeft, oRight];
};
...
function fComparisonExpr_GeneralComp(oExpr, oContext) {
	...
	for (var nLeftIndex = 0, nLeftLength = oLeft.length, bLeft, vLeft; (nLeftIndex < nLeftLength) &&!bResult; nLeftIndex++) {
		for (var nRightIndex = 0, nRightLength = oRight.length, bRight, vRight; (nRightIndex < nRightLength) &&!bResult; nRightIndex++) {
			...
			if (bLeft && bRight) {
				// cast xs:untypedAtomic to xs:string, or to xs:decimal when
				// both are written as plain numbers — see the note above
				var aPair	= fComparisonExpr_castUntypedPair(vLeft, vRight);
				vLeft	= aPair[0];
				vRight	= aPair[1];
			}
			else {
...
//<-Debug
	...
	// XForms-compatible: cast xs:untypedAtomic to the other operand's type
	// (XPath 2.0 strict: always cast to xs:string; relaxed for XForms where
	//  schema-less instances produce xs:untypedAtomic for all values)
	var bLeft	= vLeft instanceof cXSUntypedAtomic;
	var bRight	= vRight instanceof cXSUntypedAtomic;
	if (bLeft && bRight) {
		var aPair	= fComparisonExpr_castUntypedPair(vLeft, vRight);
		vLeft	= aPair[0];
		vRight	= aPair[1];
