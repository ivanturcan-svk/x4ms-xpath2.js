...
exports.numericUnaryMinus = function(oRight) {
	// Returns a new value rather than negating in place. A variable reference
	// hands out the object held in scope itself, so writing back through the
	// operand changed the variable: $v * -$v answered 100 instead of -100, and
	// $v was left holding -10 for whatever came after.
	var oExact	= fExact(oRight);
	if (oExact)
		return oRight instanceof cXSInteger
				? cXSInteger.fromParts(-oExact.digits, oExact.scale)
				: cXSDecimal.fromParts(-oExact.digits, oExact.scale);
	if (oRight instanceof cXSFloat)
		return new cXSFloat(-oRight.valueOf());
	if (oRight instanceof cXSDouble)
		return new cXSDouble(-oRight.valueOf());
	return new cXSDecimal(-oRight.valueOf());
};


// 6.3 Comparison Operators on Numeric Values
//
// The three comparisons read the values, not the doubles those values happen
// to round to. Going through valueOf() made any two decimals that share a
// double compare equal, so 0.1 and 0.10000000000000000001 were the same number
// and two 30-digit integers one apart were too — the very widths xs:decimal
// exists to carry.
//
// When either operand is an xs:double or xs:float the comparison is a double
// comparison, which is what promotion asks for: Functions and Operators,
// section "Comparison Operators on Numeric Values".

// -1, 0 or 1 when both operands are exact; null when at least one is not and
// the double path has to answer.
function fCompareExact(oLeft, oRight) {
	var oExact	= fExact(oLeft),
		oOther	= fExact(oRight);
	if (!oExact || !oOther)
		return null;
	var oAligned	= fAlign(oExact, oOther);
	return oAligned.left < oAligned.right ?-1 : oAligned.left > oAligned.right ? 1 : 0;
};

// op:numeric-equal($arg1 as numeric, $arg2 as numeric) as xs:boolean
exports.numericEqual = function(oLeft, oRight) {
	var nOrder	= fCompareExact(oLeft, oRight);
	return new cXSBoolean(nOrder === null ? oLeft.valueOf() == oRight.valueOf() : nOrder == 0);
};
