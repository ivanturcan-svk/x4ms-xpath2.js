...
cStaticContext.prototype.getFunction		= function(sUri) {
	var aMatch	= sUri.match(rStaticContext_uri);
	if (aMatch) {
		// Per-instance override takes priority over global system functions
		if (this.functions[sUri])
			return this.functions[sUri];
		return aMatch[1] == cStaticContext.NS_XPF ? cStaticContext.functions[aMatch[2]] : null;
	}
};
