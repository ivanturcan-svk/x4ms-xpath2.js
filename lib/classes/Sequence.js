...
/*
	A node's atomized value takes the type the host says the node carries.

	XForms 2.0, chapter "Expressions", section "Typed Values": a node bound by
	a bind with a type MIP carries that type into every expression that reads
	it. Without it, sum() over two money fields is a double sum and 2.01 + 0.01
	is 2.0199999999999996 — which is what xs:double means and not something the
	arithmetic can fix.

	The host answers through DOMAdapter.getTypeAnnotation(); the default answers
	null and nothing below runs. The QName it returns is looked up in the static
	context's data type registry, so the whole type set is reachable and the
	engine holds no table of its own.

	⚠️ The cast is allowed to fail and the failure is silent. XForms 2.0,
	section "Validity" says a value that does not match the declared type may
	still be stored in the node — a half-typed number in a half-filled form is
	ordinary. Throwing here would stop the expression, and with it the form.
*/
function fSequence_annotate(oContext, oNode, oUntyped) {
	var oAdapter	= oContext.DOMAdapter;
	if (!oAdapter.getTypeAnnotation)
		return oUntyped;

	var sType	= oAdapter.getTypeAnnotation(oNode);
	if (!sType)
		return oUntyped;

	var cType	= oContext.staticContext && oContext.staticContext.getDataType(sType);
	if (!cType || !cType.cast)
		return oUntyped;

	try {
		return cType.cast(oUntyped);
	}
	catch (oError) {
		return oUntyped;
	}
...
cSequence.atomize = function(oSequence1, oContext) {
	...
	for (var nIndex = 0, nLength = oSequence1.length, oItem, vItem; nIndex < nLength; nIndex++) {
		...
		if (oContext.DOMAdapter.isNode(oItem)) {
			...
			if (vItem != null)
				vItem	= fSequence_annotate(oContext, oItem, vItem);
		}
		// Base types
		else
