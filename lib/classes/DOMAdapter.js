...
/*
	The type the host says this node carries, or null for "no type".

	The XPath data model gives every node a type annotation; a document with no
	schema gets xs:untyped, and this engine had that case wired in rather than
	asked. A host that does know — an XForms processor reads it off the type MIP
	of the bind — answers here and the value atomizes as that type instead of
	xs:untypedAtomic.

	The answer is an expanded QName in Clark notation, "{namespace}localName",
	which is the key the static context's data type registry already takes. That
	keeps the whole type set reachable, user-defined types included, and keeps
	this engine from learning anything about the host: it asks one question about
	the data, and the adapter is the abstraction over the data.

	Answering null — what this default does — is what every embedder that knows
	nothing about types keeps doing, so the behaviour of this release is the
	behaviour of the last one until a host overrides the method.

	⚠️ atomize() calls this on every access to a node, not once per expression.
	An implementation must answer in constant time; a lookup that walks binds
	will show up in every refresh.
*/
cDOMAdapter.prototype.getTypeAnnotation	= function(/* oNode */) {
	return null;
};
