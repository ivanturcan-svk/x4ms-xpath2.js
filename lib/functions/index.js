...
// numeric — these four return the type they were given, so the argument must
// reach them untouched; see the note in functions/numeric.js
cStaticContext.defineSystemFunction("abs", [[cXSAnyAtomicType, '?']], oNumeric.abs);
cStaticContext.defineSystemFunction("ceiling", [[cXSAnyAtomicType, '?']], oNumeric.ceiling);
cStaticContext.defineSystemFunction("floor", [[cXSAnyAtomicType, '?']], oNumeric.floor);
cStaticContext.defineSystemFunction("round", [[cXSAnyAtomicType, '?']], oNumeric.round);
// fn:round-half-to-even returns the type it was given — Functions and
// Operators, section "fn:round-half-to-even". Declaring xs:double here cast
// the argument before the function saw it, so a decimal was rounded as a
// binary approximation of itself.
cStaticContext.defineSystemFunction("round-half-to-even", [[cXSAnyAtomicType, '?'], [cXSInteger, '', true]], oNumeric.roundHalfToEven);
// qname
